...
int socios_alta(Socios array[], int size, int* contadorID)                          //cambiar socios
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        else
        {
            ...
            utn_getEmail("Email\n: ","\nError",1,40,1,array[posicion].eMail);

            printf("\n Posicion: %d\n ID: %d\n nombre: %s\n apellido: %s\n telefono: %s\n email: %s" ,
                   posicion, array[posicion].idUnico,array[posicion].nombre,array[posicion].apellido,array[posicion].telefono,array[posicion].eMail);
            retorno=0;
        }
    }
    return retorno;
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n varInt: %d\n varFloat: %f\n nombre: %s\n apellido: %s",
                       array[i].idUnico,array[i].varInt,array[i].varFloat,array[i].nombre,array[i].apellido);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
